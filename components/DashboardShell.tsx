...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-svh bg-cream text-ink">
      <DashboardNav />
      {/* Bottom padding clears the fixed mobile nav at every text size
       * (F7 large/x-large scale the nav too, so the clearance is in rem),
       * plus the home-indicator safe area on phones. */}
      <div className="mx-auto w-full max-w-6xl px-5 pb-[calc(env(safe-area-inset-bottom)+8rem)] pt-8 sm:px-8 lg:px-10 lg:pb-12 lg:pt-10">
        {children}
      </div>
    </div>
  );
...
function DashboardNav() {
  ...
  return (
    <nav
      ...
      className="fixed inset-x-0 bottom-0 z-30 border-t border-line bg-surface px-3 pb-[env(safe-area-inset-bottom)] pt-1 shadow-[0_-8px_24px_rgba(33,29,25,0.08)] lg:static lg:border-b lg:border-t-0 lg:px-10 lg:py-4 lg:pb-4 lg:shadow-none"
    >
